...
from Levels.level18 import level_18_game_loop
...
def level_17_game_loop():
    ...
    pygame.display.set_caption('Level 18')

    game_map = load_map(os.path.join('levels', 'level18'))

    stone7_image = pygame.image.load(os.path.join('Assets', 'stone7.png'))
    ...
    while True: 
        ...
        for layer in game_map:
            ...
            for tile in layer:
                if tile == '1':
                    display.blit(stone7_image,(x*16-scroll[0],y*16-scroll[1]))
                if tile == '2':
